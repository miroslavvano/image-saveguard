...
const detectEdges = async (
  ...
  const preBlurredImageData =
    imageData.width >= BLUR_BEFORE_EDGE_DETECTION_MIN_WIDTH
      ...
      : imageData;
  console.log('IMAGE DATA', preBlurredImageData.data.length);
  const greyscaled = await luminance(canvasCtx, preBlurredImageData);
  const sobelKernel = getFloat32Array([1, 0, -1, 2, 0, -2, 1, 0, -1]);
  return convolve(canvasCtx, greyscaled, sobelKernel, true);
};
 ...
 */
const reducedPixels = (imageData: ImageData) => {
...
export const measureBlur = async (
  canvasCtx: CanvasRenderingContext2D,
  imageData: ImageData,
) => {
  const edges = await detectEdges(canvasCtx, imageData);
  return detectBlur(reducedPixels(edges));
};
